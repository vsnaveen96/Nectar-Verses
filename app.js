...
function showCategoryList(items, type) {
    ...
    if (!items.length) {
        versesGrid.innerHTML = '<div class="no-verses">No items found</div>';
        return;
    }

    // Get verse counts for each item
    getVerses().then(verses => {
        const counts = items.reduce((acc, item) => {
            acc[item] = verses.filter(verse => {
                switch(type) {
                    case 'chand':
                        return verse.chand === item;
                    case 'book':
                        return verse.book === item;
                    case 'subject':
                        return verse.subject === item;
                    case 'tags':
                        return verse.tags?.includes(item);
                    default:
                        return false;
                }
            }).length;
        ...
        const sortedItems = items.sort((a, b) => counts[b] - counts[a]);
            ...
            <div class="category-list">
                ${sortedItems.map(item => `
                    <div class="category-list-item" data-value="${item}" data-type="${type}">
                        ...
                        <span class="category-name">${item} (${counts[item]})</span>
        ...
        versesTitle.textContent = `${type.charAt(0).toUpperCase() + type.slice(1)}s (${totalCount})`;
        ...
        categoryItems.forEach(item => {
            item.addEventListener('click', () => {
                ...
                // Update title to show selected category and its count
                const versesTitle = document.querySelector('.verses-title');
                versesTitle.textContent = `${value} (${counts[value]})`;
...
categoryCards.forEach(card => {
    card.addEventListener('click', () => {
        ...
        // Update the active filter chip
        const filterChips = document.querySelectorAll('.filter-chip');
        filterChips.forEach(chip => {
            if (chip.getAttribute('data-filter') === categoryType) {
                chip.classList.add('active');
            } else {
...
document.addEventListener('DOMContentLoaded', () => {
    // ... existing display option code ...

    // Add delete multiple functionality
    ...
    if (deleteMultipleBtn) {
        deleteMultipleBtn.addEventListener('click', () => {
            const isActive = deleteMultipleBtn.classList.contains('active');
            
            if (!isActive) {
                // Enable multiple selection mode
                deleteMultipleBtn.classList.add('active');
                enableMultipleSelection();
            } else {
                // Disable multiple selection mode
                deleteMultipleBtn.classList.remove('active');
...
// Add these new functions for multiple deletion
function enableMultipleSelection() {
    ...
    verses.forEach(verse => {
        ...
        checkbox.innerHTML = `
            <input type="checkbox" class="verse-select" data-id="${verse.dataset.id}">
        `;
        verse.insertBefore(checkbox, verse.firstChild);
        
        // Add selection styles
        verse.classList.add('selectable');
    });
    
    // Add delete selected bar
    const deleteBar = document.createElement('div');
    deleteBar.className = 'delete-selection-bar';
    deleteBar.innerHTML = `
    ...
    `;
    document.querySelector('.verses-grid').insertAdjacentElement('beforebegin', deleteBar);
    
    // Add event listeners for selection
    document.querySelectorAll('.verse-select').forEach(checkbox => {
        checkbox.addEventListener('change', () => {
            updateSelectedCount();
            // Add visual feedback when selected
            const verseItem = checkbox.closest('.verse-item');
            if (checkbox.checked) {
                verseItem.style.borderColor = 'var(--maroon)';
                verseItem.style.background = 'rgba(128, 0, 0, 0.05)';
            } else {
                verseItem.style.borderColor = '';
                verseItem.style.background = '';
            }
        });
    });
    
    // Add event listener for delete selected
    document.querySelector('.delete-selected-btn').addEventListener('click', deleteSelectedVerses);
}

function disableMultipleSelection() {
    // Remove checkboxes
    document.querySelectorAll('.verse-checkbox').forEach(checkbox => checkbox.remove());
    
    // Remove selection styles
    document.querySelectorAll('.verse-item').forEach(verse => {
        verse.classList.remove('selectable');
        verse.style.borderColor = '';
        verse.style.background = '';
    });
    
    // Remove delete bar
    const deleteBar = document.querySelector('.delete-selection-bar');
    if (deleteBar) {
        deleteBar.remove();
    }
}
...
function updateSelectedCount() {
    const selectedCount = document.querySelectorAll('.verse-select:checked').length;
...
async function deleteSelectedVerses() {
    const selectedVerses = document.querySelectorAll('.verse-select:checked');
    const verseIds = Array.from(selectedVerses).map(checkbox => checkbox.dataset.id);
    ...
    dialog.innerHTML = `
        <div class="modal-overlay">
            <div class="modal-content animate-in">
                <h2>Delete Multiple Verses</h2>
                <p>To delete these ${verseIds.length} verse${verseIds.length > 1 ? 's' : ''}, please type "Hare Krishna" in the box below:</p>
                <input type="text" id="delete-confirmation" class="confirmation-input" placeholder="Type 'Hare Krishna'" style="width: 100%; padding: 12px; margin: 16px 0; border: 1px solid var(--deep-saffron); border-radius: 8px;">
                <div class="button-group">
                    <button class="cancel-btn">Cancel</button>
                    <button class="submit-btn" style="background: var(--maroon);" disabled>Delete</button>
    ...
    deleteButton.addEventListener('click', async () => {
        if (deleteInput.value === 'Hare Krishna') {
            try {
                ...
                await renderRecentVerses();
                // Disable multiple selection mode
                document.querySelector('.delete-multiple-btn').classList.remove('active');
                disableMultipleSelection();
                showToast(`${verseIds.length} verse${verseIds.length > 1 ? 's' : ''} deleted successfully`);
            } catch (error) {
                console.error("Error deleting verses:", error);
                showToast('Error deleting verses: ' + error.message);
            }
        }
    });
